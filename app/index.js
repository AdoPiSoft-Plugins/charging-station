...
module.exports = {
  async init () {
    ...
    const opi_boards = application.hardwares.filter(h => {
    ...
    if (opi_boards.includes(hardware)) {
      var opi_board = application.hardwares.find(h => h.code === hardware).board_name
    }
  ...
  uninstall () {
    // called when you uninstall the plugin
  }
}
