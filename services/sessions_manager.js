...
exports.deviceAvailableSessions = async (device_id) => {
  return exports.list.filter(s => {
    return s.mobile_device_id === device_id && !['expired', 'stopped'].includes(s.status) && s.charging_port_id === null && !s.isExpired()
  })
}
exports.sessionsByPort = async (port_id) => {
  return exports.list.filter(s => {
    return s.charging_port_id === port_id && !['expired', 'stopped'].includes(s.status) && !s.isExpired()
  })
}
exports.startSession = async (session_id) => {
  let session = await exports.findSession(session_id)
  const { charging_port_id } = session
  ...
  var exist = exports.list.find(s => s.status === 'running' && s.charging_port_id === charging_port_id)
  if (exist) {
