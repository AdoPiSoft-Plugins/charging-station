...
class Session extends EventEmitter {
  ...
  async load() {
    var s = await dbi.models.ChargingSession.findByPk(this.id)
  ...
  async prestart () {
    if (this.status === 'running') return Promise.reject('Session is already running')
    if (this.isExpired()) return Promise.reject('Session is already expired')
    await this.load()
    const {
      time_seconds,
      expire_minutes,
    ...
    if (running_time_seconds >= time_seconds) return Promise.reject('Session is already consumed')
  ...
  isExpired() {
    if (this.db_instance) {
      ...
      throw new Error('Session Instance not loaded')
    }
  }
  async pause() {
    try {
  ...
  async mobileDevices() {
    ...
    return dbi.models.MobileDevice.findAll({
      where: {
        id: ids
      }
    })
  ...
  toJSON() {
    ...
    let { charging_port_id, status } = this
    if (this.isExpired()) {
      status = 'expired'
    }
    return {
